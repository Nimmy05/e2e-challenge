...
test.describe(`Automate the 'Addition' operation of 'Vite App'`, () => {
    ...
    test(`Should perform 'Addition' of numbers successfully`, async ({ page, calculatorPage }) => {
        ...
        await test.step(`Verify addition operation with integer numbers`, async () => {
            for (const { num1, num2, expected } of thisTestConfig.addition_integer_sets) {
        ...
        await test.step(`Verify addition operation with decimal numbers"`, async () => {
            for (const { num1, num2, expected } of thisTestConfig.addition_decimal_sets) {
                await calculatorPage.clearInput();
        ...
        await test.step(`Verify subtraction involving negative integers`, async () => {
            for (const { num1, isNum1Negative, num2, isNum2Negative, expected } of thisTestConfig.addition_neg_or_pos_integer_sets) {
                await calculatorPage.clearInput();
        ...
        await test.step(`Verify subtraction with positive/negative decimal values (±)`, async () => {
            for (const { num1, isNum1Negative, num2, isNum2Negative, expected } of thisTestConfig.addition_neg_or_pos_decimal_sets) {
                await calculatorPage.clearInput();
                await calculatorPage.clickDecimalNumber(num1, digitDataTestId);
